...
export default function ArtifactPage({ params }) {
  ...
  return (
    <main className="relative min-h-screen bg-[var(--color-ground)] px-24 pt-20 pb-28">
      <ArtifactNav prev={prev} next={next} />

      <div className="mx-auto grid max-w-[1240px] grid-cols-1 gap-16 md:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)]">
        <motion.div
          key={activeImage}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        <motion.div
          ...
          className="pt-2"
        >
          <h1 className="font-sans text-[36px] font-extrabold uppercase tracking-[-0.01em] text-[var(--color-accent)]">
            {artifact.title}
          </h1>
          ...
          <div className="fade-bottom mt-10 text-[17px] leading-[1.7] text-[var(--color-ink)]">
